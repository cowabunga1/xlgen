...
exports.createXLGen = function (path,options){
	...
	function buildSAT(bookStreamLen){
		...
		while ( totalSectCount > 128*satSectCount ||
			satSectCount > satSectCountLimit ) {
		...
		for(i=0; i < len; i++) {
			bufmsat1.i32(satSect[i]);
		}
		bufmsat1.fil(SID_FREE_SECTOR, (109 - i)*4);

		bufmsat2 =  new Buf(msatSectCount*128*4);
		for(i=0; i< msatSectCount*128; i++) {
			bufmsat2.i32(SID_FREE_SECTOR);
		}
		if(msatSectCount > 0) {
		...
		while(i < satSectCount){
			if ( (sidNum + 1) % 128 === 0 ) {
				...
				if (msatSect < msatSect2.length){
					bufmsat2.pos(sidNum*4);
					bufmsat2.i32(msatSect2[msatSect]);
				}
			}else{
				//print 'sid: ',
				bufmsat2.pos(sidNum*4);
	...
	function XLGen(){
		...
		self.addSheet = function(name){
			if(!name) {
				throw new Error('sheet name can not be blank');
			}
